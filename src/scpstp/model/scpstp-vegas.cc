...
NS_LOG_COMPONENT_DEFINE ("ScpsTpVegas");
NS_OBJECT_ENSURE_REGISTERED (ScpsTpVegas);
...
ScpsTpVegas::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ScpsTpVegas")
    ...
    .AddAttribute ("Alpha", "Lower bound of packets in network",
                   UintegerValue (2),
                   MakeUintegerAccessor (&ScpsTpVegas::m_alpha),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Beta", "Upper bound of packets in network",
                   UintegerValue (4),
                   MakeUintegerAccessor (&ScpsTpVegas::m_beta),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Gamma", "Limit on increase",
                   UintegerValue (1),
                   MakeUintegerAccessor (&ScpsTpVegas::m_gamma),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
ScpsTpVegas::Fork (void)
{
  return CopyObject<ScpsTpVegas> (this);
}

void
ScpsTpVegas::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                     const Time& rtt)
{
...
void
ScpsTpVegas::EnableVegas (Ptr<TcpSocketState> tcb)
{
...
void
ScpsTpVegas::CongestionStateSet (Ptr<TcpSocketState> tcb,
...
void
ScpsTpVegas::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
...
std::string
ScpsTpVegas::GetName () const
{
  return "ScpsTpVegas";
}

uint32_t
ScpsTpVegas::GetSsThresh (Ptr<const TcpSocketState> tcb,
